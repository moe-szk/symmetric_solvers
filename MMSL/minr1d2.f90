...
! 
!-------------------------------------------------------------------------------
subroutine minr1d2(kmax,n,nzu,nzl,iu,il,au,al,ad,ksu,ksl,b,x,eps,itmax,iopt,k)
  ...
  do j=1,itmax
     ...
     v1vec = v2vec
     if(mod(j,k)==0)then
     !if(abs((sqrt(sum(r0**2))-sqrt(sum(r1**2)))/sqrt(sum(v**2))) <1.0e-10)then
       ! write(*,*)'restarted'
        v0vec = 0.d0
        w0vec = 0.d0
        w1vec = 0.d0
