...
! 
!-------------------------------------------------------------------------------
subroutine rpmr1dtest(kmax,n,nzu,nzl,iu,il,au,al,ad,ksu,ksl,b,x,eps,itmax,iopt,&
     mu,ml,md,u)
  ...
  do j=1,itmax
     ...
     !
     if(iopt/=0) write(*,*) j,',',error,sqrt(sum((u-x)**2))/sqrt(sum(u**2))
